...
if __name__ == "__main__":
    ...
    # TRAINING
    elif config.experiment_mode == 'val':
        # Get the splits for the experiment
        seqs, splits = get_seqs_from_splits(data_path=config.data_path, train_split=config.train_splits[0], val_split=config.val_splits[0])
        # Initialize the tracker
        hicl_tracker = HICLTracker(config=config, seqs=seqs, splits=splits)
        ...
        # Train the tracker
        epoch_val_logs, epoc_val_logs_per_depth = hicl_tracker.track(dataset=hicl_tracker.val_dataset, output_path=osp.join(hicl_tracker.config.experiment_path, 'val'),
                                                                     mode='val',
                                                                     oracle=False)
        
        evaluate_mot17(tracker_path=osp.join(hicl_tracker.config.experiment_path, 'val'), split=hicl_tracker.val_split,
