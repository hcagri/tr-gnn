...
from src.utils.graph_utils import to_undirected_graph, to_lightweight_graph, to_positive_decision_graph, compute_simple_iou
from src.utils.motion_utils import compute_giou_fwrd_bwrd_motion_sim
from src.tracker.projectors import GreedyProjector, ExactProjector, HungarianProjector, HungarianProjector_full
...
class HICLTracker:
    ...
    def __init__(self, config, seqs, splits):
        ...
        if self.config.experiment_mode in ('train', 'train-cval'):
            ...
            wandb.init(
                project="thesis-light", 
                config={
                    "system_config": self.config, 
                    "model_config": mymodel_params
                },
            ...
            wandb.run.log_code(
                ".",
                include_fn=lambda path: path.endswith(".py") or path.endswith(".ipynb") or path.endswith(".yaml") or path.endswith(".sh"),
            ...
            self.model, self.optimizer, self.dataloader = self.accelerator.prepare(self.model, self.optimizer, self.train_dataloader)
            
        elif self.config.experiment_mode == 'val':
            self.loss_function = FocalLoss(logits=True, gamma=self.config.gamma)
            self.train_dataloader = self._get_train_dataloader()
            self.optimizer = self._get_optimizer()
            self.val_dataset = self._get_dataset(mode='val')
    ...
    def _get_model(self):
        ...
        return HICLNet(submodel_type=LightGlueMOT, submodel_params=mymodel_params,
    ...
    def _get_projector(self, graph):
        ...
        elif self.config.rounding_method == 'hungarian_full':
            projector = HungarianProjector_full(full_graph=graph)
        else:
    ...
    def load_pretrained_model(self):
        ...
        state_dict = torch.load(self.config.hicl_model_path)

        if self.config.load_transformers_only:
            new_state_dict = {}
            ...
            # for ix in range(len(pretrained_model.layers)):  # Assuming there are 7 layers
            #     for param in pretrained_model.layers[ix].encoder.parameters():
            #         param.requires_grad = False

            #     for param in pretrained_model.layers[ix].joint_enc.parameters():
            #         param.requires_grad = False
    ...
    def _hicl_to_curr(self, hicl_graphs):
        ...
        if self.config.do_motion:
            if curr_depth < 1:
                '''
                If we have in the lower hierarchy level, directly use the IoU information 
                between the last detection of a left tracklets and first detection of a right tracklets.
                '''
                motion_pred = None
                batch.pruning_score = compute_simple_iou(batch).squeeze()
            else:
        ...
        for curr_depth in range(max_depth):
            ...
            if curr_batch.edge_index.numel():                
                if oracle:
                    ...
                    # Produce decisions
                    appearance_sim = outputs['appearance_sim']
                    pos_preds = torch.sigmoid(outputs['classified_edges'][-1].view(-1).detach())
                    preds = appearance_sim*0.5 + pos_preds*0.5
                    preds[preds<0] = 0
                    curr_batch.edge_preds = preds
            ...
            if curr_depth < project_max_depth:  # Last layer update is not necessary for training
                for ix_graph, graph in enumerate(graph_data_list):        
                    if graph.edge_index.numel():
                        ...
                        if mode == 'train' and self.epoch < 65:
                            graph.edge_preds = graph.edge_labels
                        # Process the graph before feeding it to the projector
    ...
    def train(self):
        ...
        wandb.watch(self.model, self.loss_function, log='all', log_freq=1000)
        # Training loop
