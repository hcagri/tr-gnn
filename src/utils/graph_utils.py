...
def find_graph_time_valid_edges(node_frames, node_frames_mask, depth, frames_per_level, connectivity, pruning_method=['geometry', 'reid', 'reid']):
    ...
    inter_edge_ixs: torch.Tensor with shape (2, num_inter_edges) correspondint to edges in between same frame/hierarchy
    """
    ...
    # edge_ixs, self_edge_index, node_batch_labels, bipartite_labels = find_time_valid_connections_flowchunk(node_frames=node_frames, depth=depth, frames_per_level=frames_per_level)
    edge_ixs, node_batch_labels, bipartite_labels = find_time_valid_connections_flowchunk(node_frames=node_frames, depth=depth, frames_per_level=frames_per_level)

    return edge_ixs, node_batch_labels, bipartite_labels
...
def find_time_valid_connections_flowchunk(node_frames, depth, frames_per_level):
    """
    All time valid connections of a graph. Nodes from consecutive splits are merged together. 
    This time, non conflicting nodes from the same split are also merged together. So lower layer bad decisions can be recovered. !!! This is not the case in my implementation.

    !!! Modified: 
        In each hierarchy bipartite graph is constructed.
    """
    ...
    small_clusters = torch.floor((start_frames-min_start_frame)/prev_coverage)  # Cluster the nodes according to their start frames
    ...
    edge_ixs = [] # Edges between frames
    self_edge_index = [] # Edges within frame
    for large_start_ix, large_end_ix in zip(large_cluster_changepoints[:-1], large_cluster_changepoints[1:]):
        ...
        cluster_start_frames = torch.unique(start_frames[large_start_ix:large_end_ix])
        min_start_in_cluster = min(cluster_start_frames)
        end_f = min_start_in_cluster + coverage//2 # Assume the covarage will always be the multiple of 2
        # Loop over the end frames, !!! This is not constructing bipartite graph.
        ...
        if pasts.numel() and currs.numel(): # Returns total number of elements in a tensor!
            edge_ixs.append(torch.cartesian_prod(pasts, currs))
            # past_self_edge_ixs = torch.cartesian_prod(pasts, pasts)
            # curr_self_edge_ixs = torch.cartesian_prod(currs, currs)
            # ''' Filter out self edges from inter connections '''
            # past_self_edge_ixs = past_self_edge_ixs[past_self_edge_ixs[:, 0] != past_self_edge_ixs[:, 1]]
            # curr_self_edge_ixs = curr_self_edge_ixs[curr_self_edge_ixs[:, 0] != curr_self_edge_ixs[:, 1]]
            # self_edge_index.append(torch.cat([past_self_edge_ixs, curr_self_edge_ixs], dim=0))

    if edge_ixs:
        edge_ixs = torch.cat(edge_ixs).T
        # self_edge_index = torch.cat(self_edge_index).T
        ...
        # self_edge_index = torch.zeros((2, 0), dtype=torch.int64, device=node_frames[0].device)

    # return edge_ixs, self_edge_index, large_clusters, small_clusters
    return edge_ixs, large_clusters, small_clusters
...
def make_symmetric_edges(edge_ixs, edge_features=None):
    """
...
def compute_simple_iou(batch):
    """
    Internal Args:
        boxA: torch tensor of bounding boxes with size (N, 4)
        boxB: torch tensor of bounding boxes with size (N, 4)
    ...
    assert (row < col).all() # Edges are directed.

    boxA = batch.x_box_end[row]   # End bbox of a tracklet   (left)
    boxB = batch.x_box_start[col] # Start bbox of a tracklet (right)
    
    boxA = torch.cat([boxA[:, :2], boxA[:, :2] + boxA[:, 2:]], dim=1)
    boxB = torch.cat([boxB[:, :2], boxB[:, :2] + boxB[:, 2:]], dim=1)
    
    x11, y11, x12, y12 = np.split(boxA.detach().cpu(), 4, axis=1)
    x21, y21, x22, y22 = np.split(boxB.detach().cpu(), 4, axis=1)
    ...
    return iou.to(batch.edge_index.device)