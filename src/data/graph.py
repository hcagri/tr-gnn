...
import numpy as np
from src.utils.graph_utils import find_graph_time_valid_edges, prune_edges, compute_edge_features, assign_edge_labels, assign_edge_labels_v2, make_symmetric_edges, assign_node_ids
...
class HierarchicalGraph(Data):
    ...
    def _get_track_features(self, config):
        ...
        normalized_frames = self.x_frame - self.x_frame.min()
        max_track_len = config.frames_per_level[depth-1]
        unique_frame_ids = torch.unique(normalized_frames)

        partitioned_bbox_pos = torch.arange(0, max_track_len).repeat(config.frames_per_graph//max_track_len)[:unique_frame_ids.shape[0]]
        dict_values = dict(zip(unique_frame_ids.tolist(), partitioned_bbox_pos.tolist()))

        img_shape_ = torch.tensor([self.frame_width, self.frame_height], dtype=torch.float, device= self.x_frame.device)
        ...
        for cluster_ix in range(0, torch.max(self.map_from_init).item()+1):
            ...
            frames = normalized_frames[cluster_mask]
            bbox = self.x_bbox[cluster_mask] / torch.cat([img_shape_,img_shape_])
            whs_ratio = bbox[:, 2]/bbox[:, 3]
            feets = self.x_feet[cluster_mask] / img_shape_
            centers = self.x_center[cluster_mask] / img_shape_
            features = torch.cat([bbox, centers, feets[:, 1:], whs_ratio.unsqueeze(1), frames], dim=1)

            frames_sorted, ix_sort = torch.sort(frames)
            assert torch.all(frames == frames_sorted), 'Frames are not sorted'
            # List of all frames you want to consider

            bbox_temp_pos = torch.tensor([dict_values[key.item()] for key in frames]) # in temporal partitions
            frame_portion_pos = torch.arange(0, max_track_len) # If there is no missing frame

            filled_features = torch.zeros(frame_portion_pos.shape[0], features.shape[1], device=frames.device)
            # Create a mask for available frames
            mask = torch.tensor([pos in bbox_temp_pos for pos in frame_portion_pos])
            filled_features[mask, :] = features

            # Update the lists
            x_track.append(filled_features)
            mask_lst.append(mask)

        track_feats = torch.stack(x_track, dim=0) # shape[num_tracklets, tracklet_len (in specified depth), feature_dim = 10]
        mask = torch.stack(mask_lst, dim=0).to(track_feats.device)
        return track_feats, mask
    ...
    def construct_curr_graph_nodes(self, config):
        ...
        if self.curr_depth > 0:
            x_track, tracklet_mask = self._get_track_features(config)
        else:
            img_shape_ = torch.tensor([self.frame_width, self.frame_height], dtype=torch.float, device= self.x_frame.device)
            tracklet_mask = -1
            whs_ratio = x_bbox[0][:, 2]/x_bbox[0][:, 3]
            x_frame_norm = x_frame[0] - x_frame[0].min()
            x_center_ = x_center[0] / img_shape_
            x_feet_ = x_feet[0] / img_shape_
            x_bbox_ = x_bbox[0] / torch.cat([img_shape_,img_shape_])
            x_track = torch.cat([x_bbox_, x_center_, x_feet_[:, 1:], whs_ratio.unsqueeze(1), x_frame_norm], dim=1).unsqueeze(1) # shape: [num_detections, 1, feature_dim]
        ...
        # raw_edge_index, self_edge_index, node_batch_labels, bipartite_labels = find_graph_time_valid_edges(node_frames=x_frame, node_frames_mask=x_frame_mask, 
        #                                              depth=self.curr_depth, frames_per_level=self.frames_per_level, 
        #                                              connectivity=config.connectivity)

        raw_edge_index, node_batch_labels, bipartite_labels = find_graph_time_valid_edges(node_frames=x_frame, node_frames_mask=x_frame_mask, 
                                                depth=self.curr_depth, frames_per_level=self.frames_per_level, 
                                                connectivity=config.connectivity)
        
        curr_graph = Graph(x=x_node, edge_index=raw_edge_index,
                            x_reid=x_reid,y_id=y_id, fwrd_vel=fwrd_vel, bwrd_vel=bwrd_vel, 
                            ...
                            bipartite_labels = bipartite_labels,
                            x_track = x_track, tracklet_mask = tracklet_mask,  **motion_features)
    ...
    def add_edges_to_curr_graph(self, config, curr_graph):
        ...
        # edge_ixs = curr_graph.edge_index
        # self_edge_ixs = curr_graph.self_edge_index
        ...
        # self_edge_ixs, _ = prune_edges(edge_ixs=curr_graph.self_edge_index, node_frames=x_frame, node_reids=x_reid, top_k_nns=10, 
        #                                 depth=self.curr_depth, node_feets=x_feet, graph_pruning_score=curr_graph.pruning_score, pruning_method = config.pruning_method_self,
        #                                 reid_sim_fn=reid_sim_fn)
        ...
        if config.symmetric_edges:
            ...
            # self_edge_ixs, _ = make_symmetric_edges(edge_ixs=self_edge_ixs)

        # LABELS - if train/val
        edge_labels, edge_mask = assign_edge_labels(node_ids=curr_graph.y_id, edge_ixs=edge_ixs, node_frames=x_frame)
        # edge_labels = assign_edge_labels_v2(node_ids=curr_graph.y_id, edge_ixs=edge_ixs, node_frames=x_frame)

        # Placeholder for edge predictions
        edge_preds = torch.zeros((edge_ixs.shape[1]), dtype=torch.float)

        # img_shape_ = torch.tensor([self.frame_width, self.frame_height], dtype=torch.float, device= edge_ixs.device)
        # curr_graph.x_box_start = curr_graph.x_box_start/torch.cat([img_shape_,img_shape_])
        # curr_graph.x_box_end   = curr_graph.x_box_end/torch.cat([img_shape_,img_shape_])

        curr_graph = Graph(x=curr_graph.x, edge_index=edge_ixs, edge_features = edge_features, 
                           x_track = curr_graph.x_track,tracklet_mask=curr_graph.tracklet_mask, edge_labels=edge_labels, y_id=curr_graph.y_id, edge_preds=edge_preds, 
                           edge_mask=edge_mask, bipartite_labels= curr_graph.bipartite_labels)
        return curr_graph
